...
#     #             print(f"Rendering with OpenCV for view {nv}...")

#     #             # 創建地面網格（模擬 create_ground）
#     #             if self.render_mode == 'ground':
#     #                 # 初始化白色背景
#     #                 vis = np.zeros((self.shape[0], self.shape[1], 3), dtype=np.uint8) + 255
#     #                 focal = min(self.shape) * 1.2
#     #                 K = np.array([
#     #                     [focal, 0, vis.shape[1]/2],
#     #                     [0, focal, vis.shape[0]/2],
#     #                     [0, 0, 1]
#     #                 ])
                    
#     #                 # 生成地面網格（棋盤格）
#     #                 step, xrange, yrange = 1, 10, 10
#     #                 white, black = (255, 255, 255), (128, 128, 128)
#     #                 ground_verts = []
#     #                 ground_faces = []
#     #                 for i in range(-xrange, xrange + 1, step):
#     #                     for j in range(-yrange, yrange + 1, step):
#     #                         ground_verts.append([i, j, -0.05])
#     #                 ground_verts = np.array(ground_verts, dtype=np.float32)
#     #                 for i in range(xrange * 2 // step):
#     #                     for j in range(yrange * 2 // step):
#     #                         idx = i * (yrange * 2 // step + 1) + j
#     #                         ground_faces.append([idx, idx + 1, idx + yrange * 2 // step + 2, idx + yrange * 2 // step + 1])
#     #                 ground_faces = np.array(ground_faces, dtype=np.int32)

#     #                 # 投影地面到 2D
#     #                 verts_h = ground_verts @ R.T + T.reshape(1, 3)
#     #                 verts_2d = verts_h @ K.T
#     #                 verts_2d = verts_2d[:, :2] / verts_2d[:, 2:]
                    
#     #                 # 繪製地面棋盤格
#     #                 for idx, f in enumerate(ground_faces):
#     #                     pts = verts_2d[f].astype(int)
#     #                     color = white if (idx // (yrange * 2 // step) + idx % (yrange * 2 // step)) % 2 == 0 else black
#     #                     cv2.fillPoly(vis, [pts], color)

#     #             # 渲染人體網格
#     #             if vert.ndim == 3:  # 多人 (P, V, 3)
#     #                 if len(pids) == 0:
#     #                     pids = list(range(vert.shape[0]))
#     #                 for ipid, pid in enumerate(pids):
#     #                     verts_h = vert[ipid] @ R.T + T.reshape(1, 3)
#     #                     verts_2d = verts_h @ K.T
#     #                     verts_2d = verts_2d[:, :2] / verts_2d[:, 2:]
...
#     #                     for f in faces:
#     #                         pts = verts_2d[f].astype(int)
#     #                         cv2.fillPoly(vis, [pts], color=(0, 255, 0))  # 綠色填充
#     #                         cv2.polylines(vis, [pts], isClosed=True, color=(0, 128, 0), thickness=1)  # 深綠色邊框

#     #             elif vert.ndim == 2:  # 單人 (V, 3)
#     #                 verts_h = vert @ R.T + T.reshape(1, 3)
#     #                 verts_2d = verts_h @ K.T
#     #                 verts_2d = verts_2d[:, :2] / verts_2d[:, 2:]
...
#     #             meshes = {}
#     #             if vert.ndim == 2:
#     #                 meshes[0] = {'vertices': vert, 'faces': faces, 'id': 0, 'name': 'human_0'}
#     #             else:
#     #                 if len(pids) == 0:
#     #                     pids = list(range(vert.shape[0]))
#     #                 for ipid, pid in enumerate(pids):
#     #                     meshes[pid] = {'vertices': vert[ipid], 'faces': faces, 'id': pid, 'name': f'human_{pid}'}
#     #             ret = plot_meshes(vis, meshes, K, R, T, mode=self.render_mode)
                ...
                if index >= result['frames'][0] and index <= result['frames'][-1]:
                    ...
                    for key in ['Rh', 'Th', 'poses', 'shapes','expression']:
                        ...
                        if result['params'][key].shape[0] == 1:
                            ...
                            res_item[key] = result['params'][key][frame_rel:frame_rel+1]
                    results_frame.append(res_item)
